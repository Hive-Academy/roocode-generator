...
// Refined mock for MemoryBankCommandHandler
jest.mock('@commands/memory-bank-command-handler', () => ({
  MemoryBankCommandHandler: jest.fn().mockImplementation(() => ({
    handle: jest.fn().mockImplementation(() => Promise.resolve(Result.ok(undefined))),
    // Add other methods if needed
  })),
}));
...
//       expect(result.isErr()).toBe(true);
//       expect(result.error?.message).toBe(`No registration found for token: ${UNREGISTERED_TOKEN}`);
//     });
...
//       const result = container.resolve<any>(ERROR_TOKEN);
//       expect(result.isErr()).toBe(true);
//       // The error might be wrapped, check the original error if needed
//       expect(result.error?.message).toContain('Singleton constructor failed!');
//     });
//   });
...
//   //     const memoryBankHandlerResult = container.resolve<MemoryBankCommandHandler>(
//   //       'MemoryBankCommandHandler'
//   //     );
//   //     expect(memoryBankHandlerResult.isOk()).toBe(true);
//   //     expect(memoryBankHandlerResult.value).toBeDefined();
//   //     expect(memoryBankHandlerResult.value!.handle).toBeDefined(); // Check refined mock method
//   //
