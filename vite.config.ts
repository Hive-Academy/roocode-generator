...
export default async () => {
  ...
  const checker = (await import('vite-plugin-checker')).default; // Default export
  ...
  return defineConfig({
    plugins: [
      ...
      // Removed viteStaticCopy plugin configuration
    ],
    resolve: {
      alias: {
    ...
    build: {
      ...
      rollupOptions: {
        ...
        external: [
          ...
          // Explicitly externalize tree-sitter JS modules
          'node-tree-sitter',
...
}; // Closing brace for async function wrapper
